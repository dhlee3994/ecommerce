...
sequenceDiagram
    ...
    user->>cartItem: 장바구니 상품 구매 요청
    activate cartItem
    
    cartItem->>member: 회원 정보 요청
    activate member
    break 회원이 아니라면
        member--xcartItem: 회원아님 예외
        cartItem--xuser: 회원아님 예외
    end
    member-->>cartItem: 회원 정보 응답
    deactivate member

    cartItem->>order: 주문 생성 요청
    activate order

    note over order,pointHistory: 트랜잭션
    
    order->>cartItem: 장바구니 상품 목록 조회 요청(락 획득)
    ...
    loop 장바구니에 담긴 상품 목록
        ...
        status--xorder: 판매종료 상품 예외
            order--xcartItem: 판매종료 상품 예외
            cartItem--xuser: 판해종료 상품 예외
        end
    ...
    stock->>stockHistory: 모든 재고 차감 내역 추가 요청
    activate stockHistory
    stockHistory->>stock: 모든 재고 차감 내역 추가 완료 응답
    deactivate stockHistory
    
    order->>order: 주문 전체에 적용 가능한 쿠폰 적용

    order->>couponUseHistory: 사용한 쿠폰들의 사용 내역 추가 요청
    activate couponUseHistory
    couponUseHistory->>order: 사용한 쿠폰들의 사용 내역 추가 완료 응답
    ...
    break 쿠폰 적용한 최종 결제 금액이 보유 포인트보다 많으면
        ...
        order--xcartItem: 잔액 부족 예외
        cartItem--xuser: 잔액 부족 예외
    end
    ...
    order->>cartItem: 주문된 상품을 장바구니에서 삭제 요청
    cartItem-->>order: 주문된 상품을 장바구니에서 삭제 완료 응답
    ...
    order->>cartItem: 주문 생성 완료 응답
    deactivate order
    
    cartItem->>user: 주문 완료 응답
    deactivate cartItem
```
