...
sequenceDiagram
    ...
    participant couponUseHistory as 발급된 쿠폰 사용 히스토리
    ...
    stock->>stockHistory: 재고 차감 내역 추가 요청
    activate stockHistory
    stockHistory->>stock: 재고 차감 내역 추가 완료 응답
    ...
    couponIssue->>couponUseHistory: 쿠폰 사용 내역 추가 요청
    activate couponUseHistory
    couponUseHistory->>couponIssue: 쿠폰 사용 내역 추가 완료 응답 
    deactivate couponUseHistory
    ...
    point->>pointHistory: 포인트 차감 내역 추가 요청
    activate pointHistory
    pointHistory->>point: 포인트 차감 내역 추가 완료 응답
    deactivate pointHistory
    
    order->>orderHistory: 주문 내역 추가 요청
    activate orderHistory
    orderHistory->>order: 주문 내역 추가 완료 응답
